...
namespace Test_PJS5_CSharp
{
    public class TestRobot
    {
        ...
        [Fact] 
        public void Test_DealingDamageWithDifferentWeapons()
        {
        ...
        [Fact]
        public void Test_DifferentsWeaponIsUsable()
        {
        ...
        [Fact]
        public void Test_DifferentWeaponsHitChance()
        {
            IWeapon pProjectileWeapon = new PROJECTILE_WEAPON(1, "Basic Projectile Weapon", 3, 1, 1, 25, 60, 50, 1);
            IWeapon pMeleeWeapon = new MELEE_WEAPON(1, "Basic Melee Weapon", 3, 1, 1, 25, 100, 0);
            IWeapon pNormalWeapon = new NORMAL_WEAPON(1, "Basic Normal Weapon", 3, 1, 1, 100, 80, 40);
        ...
        [Fact]
        public void Test_AttackTargetIsValid()
        {
            //Checking if all the parts of the player robot is attackable
            for(int i = 0; i < 4; i++)
            {
                robotPlayer.AttackTargetIsValid(i+1).Should().BeTrue();
            }
            //Trying to attack a non-existent part
            robotPlayer.AttackTargetIsValid(5).Should().BeFalse();

            //Breaking all the parts of the player robots to check if they are still attackable
            for (int i = 0; i <= 1; i++) robotBot.DealDamage(robotPlayer, 1, 1);
            for (int i = 0; i <= 1; i++) robotBot.DealDamage(robotPlayer, 1, 2);
            for (int i = 0; i <= 1; i++) robotBot.DealDamage(robotPlayer, 1, 3);
            robotBot.DealDamage(robotPlayer, 1, 4);
            
            //Now, all the parts are unattackable.
            for(int i = 0; i < 4; i++)
            {
                robotPlayer.AttackTargetIsValid(i+1).Should().BeFalse();
            }

        }

        [Fact]
        public void Test_RepairLifeAndArmorTargetIsValid()
        {
            //Checking if the player robot parts cannot be repaired
            for (int i = 0; i < 4; i++)
            {
                robotPlayer.RepairLifeTargetIsValid(i + 1).Should().BeFalse();
                robotPlayer.RepairArmorTargetIsValid(i + 1).Should().BeFalse();
            }
            //Trying to repair a non-existent part
            robotPlayer.RepairLifeTargetIsValid(5).Should().BeFalse();
            robotPlayer.RepairArmorTargetIsValid(5).Should().BeFalse();

            //Breaking all the player robot parts
            for (int i = 0; i <= 1; i++) robotBot.DealDamage(robotPlayer, 1, 1);
            for (int i = 0; i <= 1; i++) robotBot.DealDamage(robotPlayer, 1, 2);
            for (int i = 0; i <= 1; i++) robotBot.DealDamage(robotPlayer, 1, 3);
            robotBot.DealDamage(robotPlayer, 1, 4);

            //Now checking if the player robot parts can be repaired
            for (int i = 0; i < 4; i++)
            {
                robotPlayer.RepairLifeTargetIsValid(i + 1).Should().BeTrue();
                robotPlayer.RepairArmorTargetIsValid(i + 1).Should().BeTrue();
            }
        }
    }
}